...
from runtime.localize_anything.generation import collect_generated_handoff, create_draft_request, create_generation_handoff  # noqa: E402
...
from runtime.localize_anything.xcstrings_adapter import extract_segments, stage_rebuild, validate_pair  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the pinned IceCubesApp Xcode String Catalog benchmark")
    parser.add_argument("workspace", type=Path)
    parser.add_argument("--target-locale", default=CONFIG["locales"]["target"])
    parser.add_argument("--max-segments", type=int, default=CONFIG["expected_results"]["default_max_segments"])
...
def run_benchmark(
    ...
    qa = validate_pair(source_path, staged_target, target_locale)
    write_json(evidence_dir / "xcstrings-qa.json", qa)
    packaged = package_delivery(
        state_dir,
        staging_dir,
        deliveries_dir,
        [evidence_dir / "generation-collect.json", evidence_dir / "xcstrings-qa.json"],
        "draft_package",
        CONFIG["id"],
    )
