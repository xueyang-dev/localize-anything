...
from runtime.localize_anything.ios_strings_adapter import extract_segments, stage_rebuild, validate_pair  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the pinned Signal iOS Localizable.strings benchmark")
...
def run_benchmark(
    ...
    write_json(evidence_dir / "ios-qa.json", qa)
    packaged = package_delivery(
        state_dir,
        staging_dir,
        deliveries_dir,
        [evidence_dir / "generation-collect.json", evidence_dir / "ios-qa.json"],
        "draft_package",
        CONFIG["id"],
    )
