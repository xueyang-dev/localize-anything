...
from .provider_smoke_closure import read_provider_smoke_closure_artifact
...
from .workbench_run import WorkbenchRunError, build_workbench_run_view
from .workbench_ui import WORKBENCH_HTML
...
def _handler_factory(state: WorkbenchState) -> type[BaseHTTPRequestHandler]:
    class LocalizeAnythingUIHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            try:
                if parsed.path in {"/", "/generate", "/review", "/sessions", "/settings"}:
                ...
                if parsed.path == "/api/health":
                    ...
                    return
                if parsed.path == "/api/workbench-run":
                    self._handle_workbench_run_query(parsed.query)
                    return
                ...
                for endpoint, artifact_name in (
                    ...
                    if parsed.path == endpoint:
                        ...
                        return
                for endpoint, artifact_name in (
                    ("/api/provider-smoke-closure-report", "provider_smoke_closure_report"),
                    ("/api/provider-smoke-release-boundary-audit", "provider_smoke_release_boundary_audit"),
                    ("/api/provider-smoke-evidence-manifest", "provider_smoke_evidence_manifest"),
                    ("/api/provider-smoke-remaining-blockers", "provider_smoke_remaining_blockers"),
                    ("/api/provider-smoke-next-step-decision", "provider_smoke_next_step_decision"),
                ):
                    if parsed.path == endpoint:
                        self._handle_workflow_artifact_query(parsed.query, artifact_name, lambda state_dir, name=artifact_name: read_provider_smoke_closure_artifact(state_dir, name))
                        return
                ...
                raise WorkbenchRunError(
                    "NOT_FOUND",
                    "The requested Workbench route does not exist.",
                    status=HTTPStatus.NOT_FOUND,
                    recoverable=False,
                )
            except WorkbenchRunError as exc:
                self._send_json(exc.envelope(), HTTPStatus(exc.status))
            except (OSError, ValueError, json.JSONDecodeError) as exc:
                self._send_json(_exception_error_envelope(exc), HTTPStatus(_exception_status(exc)))

        def do_POST(self) -> None:
            parsed = urlparse(self.path)
            try:
                payload = self._read_json_body()
                if parsed.path == "/api/quickstart-demo":
                    self._handle_quickstart_demo()
                    return
        ...
        def _handle_workbench_run_query(self, query: str) -> None:
            params = parse_qs(query)
            project_value = (params.get("project") or [""])[0]
            run_id = (params.get("run_id") or [""])[0]
            view = build_workbench_run_view(project_value, run_id)
            state.add_allowed_root(project_value)
            for pointer in view.get("artifacts", []):
                if isinstance(pointer, dict):
                    state.add_allowed_root(pointer.get("path"))
            self._send_json(
                {
                    "status": "pass",
                    "status_code": "pass",
                    "status_family": "success",
                    "message_code": "WORKBENCH_RUN_AVAILABLE",
                    "message_params": {},
        ...
        def _handle_quickstart_demo(self) -> None:
            output_root = Path(gettempdir()) / "localize-anything-workbench-demos" / f"demo-{uuid.uuid4().hex[:12]}"
            summary = run_quickstart_demo(output_root=output_root)
            state.add_allowed_root(output_root)
            state.add_allowed_root(summary.get("copied_project"))
            self._send_json({"status": "pass", "demo_summary": summary})
        ...
        def _handle_artifact_state_query(self, query: str) -> None:
            ...
            artifact_path = state_dir / "artifact-state.json"
            try:
                value = read_artifact_state(state_dir)
            except ValueError:
                if artifact_path.is_file():
                    raise
                raise WorkbenchRunError(
                    "ARTIFACT_MISSING",
                    f"Artifact state does not exist: {artifact_path}",
                    status=HTTPStatus.NOT_FOUND,
                    recoverable=True,
                    actions=["RETRY", "OPEN_SESSIONS"],
                    artifact=artifact_path.as_posix(),
                ) from None
            self._send_json({"status": "pass", "state_dir": state_dir.as_posix(), "artifact_state": value})
        ...
        def _handle_readiness_authorization_query(self, query: str, artifact: str) -> None:
            ...
            readers = {
                "matrix": ("readiness_authorization_matrix", "readiness-authorization-matrix.json", read_readiness_authorization_matrix),
                "manual_followup": ("manual_followup_gap_report", "manual-followup-gap-report.json", read_manual_followup_gap_report),
                "apply": ("apply_readiness_report", "apply-readiness-report.json", read_apply_readiness_report),
                "delivery": ("delivery_readiness_report", "delivery-readiness-report.json", read_delivery_readiness_report),
            }
            key, filename, reader = readers[artifact]
            artifact_path = state_dir / filename
            try:
                value = reader(state_dir)
            except ValueError:
                if artifact_path.is_file():
                    raise
                raise WorkbenchRunError(
                    "ARTIFACT_MISSING",
                    f"Readiness artifact does not exist: {artifact_path}",
                    status=HTTPStatus.NOT_FOUND,
                    recoverable=True,
                    actions=["RETRY", "OPEN_SESSIONS"],
                    artifact=artifact_path.as_posix(),
                ) from None
            self._send_json({"status": "pass", "state_dir": state_dir.as_posix(), key: value})
        ...
        def _handle_workbench_action(self, payload: dict[str, Any]) -> None:
            ...
            _require_matching_run_identity(payload, action)
...
def _exception_error_envelope(exc: Exception) -> dict[str, Any]:
    if isinstance(exc, FileNotFoundError):
...
def _require_matching_run_identity(payload: dict[str, Any], action: dict[str, Any]) -> None:
    run_id = _optional_string(payload.get("run_id"))
    action_run_id = _optional_string(action.get("run_id"))
    if run_id and action_run_id and run_id != action_run_id:
        raise WorkbenchRunError(
            "RUN_STATE_MISMATCH",
            "The action and selected run identities do not match.",
