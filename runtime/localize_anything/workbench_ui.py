...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .error-actions { display: flex; flex-wrap: wrap; justify-content: flex-end; gap: 8px; flex: 0 0 auto; }
...
<body>
  ...
  <main id="main-content" tabindex="-1">
    ...
    <div id="errorStatus" class="error-notice" role="alert"><span class="error-copy" id="errorMessage"></span><span class="error-actions"><button id="errorRetry" type="button" onclick="retryLastAction()" data-i18n="common.retry" hidden>Retry</button><button type="button" onclick="clearError()" data-i18n="common.dismiss">Dismiss</button></span></div>
    ...
    <section class="page" data-page="/review" hidden>
      ...
      <div class="review-dashboard">
        ...
        <div id="reviewContent" class="stack" hidden>
          ...
          <div class="card" id="reviewNextAction" hidden>
            <div class="card-header"><div><h2 data-i18n="review.nextActionTitle">Next step</h2><p class="card-copy" data-i18n="review.nextActionCopy">The handoff is ready for the next localization step.</p></div><span class="status-chip progress" data-i18n="status.awaiting_llm_responses">Awaiting generated responses</span></div>
            <div class="card-body"><p id="reviewNextActionCopy" data-i18n="review.awaitingActionCopy">Continue to Prepare to import generated responses. The Workbench does not call a provider automatically.</p><div class="inline-actions"><button class="primary" id="reviewContinueButton" type="button" onclick="continueLocalization(currentRunView && currentRunView.run_id)" data-i18n="review.continueLocalization">Continue localization</button></div></div>
          </div>
  ...
  <script>
    ...
    const TRANSLATIONS = {
      en: {
        ...
        "routing.detectedNature":"Detected project nature","routing.resources":"Resources","routing.sourceFiles":"Source files","routing.adapters":"Adapters","routing.primaryRoute":"Primary route","routing.viewFiles":"View recognized files ({count})","routing.path":"Path","routing.adapter":"Adapter","routing.moreFiles":"{count} more recognized files are available in the inspection result.","routing.preflight":"Recommended preflight: {mode}","routing.sourceTruthExclusion":"Excluded {count} Android locale-reference files from source selection. They remain visible for review but cannot be source truth.","routing.sourceFilesMissing":"No safe source-of-truth files were recognized. Add a base/default resource before preparing a handoff.",
        ...
        "operation.kicker":"Project intake","operation.openingTitle":"Opening project","operation.openingDetail":"Waiting for the local folder selection.","operation.choose":"Choose","operation.recognize":"Recognize","operation.prepare":"Prepare","operation.write":"Write","operation.ready":"Ready","operation.chooseTitle":"Choose a local project","operation.chooseDetail":"Use the system folder window to select the project you want to recognize.","operation.recognizingTitle":"Recognizing the project","operation.recognizingDetail":"Reading supported resources and adapter evidence. Large projects can take a little longer.","operation.recognizedTitle":"Project recognized","operation.recognizedDetail":"{count} supported resource files are ready to configure.","operation.readingFilesTitle":"Reading selected files","operation.readingFilesDetail":"Preparing {current} of {total} files for a temporary project.","operation.importingTitle":"Importing and recognizing","operation.importingDetail":"Creating the temporary project and reading supported resources.","operation.importedTitle":"Project imported","operation.preparingTitle":"Preparing generation handoff","operation.preparingDetail":"Validating recognized source files and building the handoff. Larger projects can take a little longer.","operation.validatingTitle":"Working through the recognized project","operation.validatingDetail":"Inspecting source segments and writing evidence. Keep this window open.","operation.preparedTitle":"Handoff ready","operation.preparedDetail":"The handoff is ready to review.",
      ...
      "zh-CN": {
        ...
        "routing.detectedNature":"识别到的项目性质","routing.resources":"资源文件","routing.sourceFiles":"源文件","routing.adapters":"适配器","routing.primaryRoute":"主要路由","routing.viewFiles":"查看识别到的文件（{count}）","routing.path":"路径","routing.adapter":"适配器","routing.moreFiles":"检查结果中还有 {count} 个已识别文件。","routing.preflight":"建议的预检模式：{mode}","routing.sourceTruthExclusion":"已将 {count} 个 Android 语言目录文件排除在源文件选择之外；它们仍会保留在识别结果中供复核，但不能作为源真值。","routing.sourceFilesMissing":"未识别到安全的源真值文件。请先添加默认语言资源，再准备交接包。",
        ...
        "review.kicker":"复核","review.title":"本次运行的准确证据","review.lead":"就绪状态、交付、声明、操作和产物都严格限定在当前选择的运行记录。","review.notSelected":"未选择","review.emptyTitle":"选择一条运行记录","review.emptyMessage":"请选择一条运行记录，或运行安全演示后进入复核。","review.openSessions":"打开运行记录","review.loadErrorTitle":"无法载入这次运行","review.loadErrorMessage":"无法载入所选运行记录。","review.nextActionTitle":"下一步","review.nextActionCopy":"交接包已就绪，可以继续本地化流程。","review.awaitingActionCopy":"进入“准备”页导入生成结果。工作台不会自动调用服务商或模型。","review.continueLocalization":"继续本地化",
        ...
        "review.scopeCurrent":"同一次运行的当前项目证据（缺少快照）","review.scopeSelected":"所选运行的快照","review.runtimeEvidenceAvailable":"运行时证据可用。","review.evidenceUnavailable":"证据不可用","review.awaitingResponses":"导入生成结果后，复核就绪和产物证据才会生成。","review.artifactEvidenceUnavailable":"产物状态证据不可用","review.artifactCounts":"已索引 {indexed} · 过期 {stale} · 缺少必需项 {missing} · 需复核 {review}","review.operations":"计划执行 {count} 项操作","review.applyAvailable":"应用计划可用","review.applyAuthorization":"应用输出需要单独授权。","review.applyNotExposed":"此工作台不提供应用执行入口。","review.newerStateRisk":"项目已有更新状态；此页面仍严格限定在所选历史运行。","review.noLimitations":"未找到已记录的限制产物，但这不能证明本次运行没有风险。","review.moreRisks":"原始产物中还有 {count} 条已记录内容。","review.recorded":"已记录",
        ...
        "operation.kicker":"项目导入","operation.openingTitle":"正在打开项目","operation.openingDetail":"等待选择本地文件夹。","operation.choose":"选择","operation.recognize":"识别","operation.prepare":"准备","operation.write":"写入","operation.ready":"就绪","operation.chooseTitle":"选择本地项目","operation.chooseDetail":"请在系统文件夹窗口中选择要识别的项目。","operation.recognizingTitle":"正在识别项目","operation.recognizingDetail":"正在读取支持的资源和适配器证据；大型项目可能需要更长时间。","operation.recognizedTitle":"项目识别完成","operation.recognizedDetail":"已有 {count} 个受支持的资源文件可供配置。","operation.readingFilesTitle":"正在读取所选文件","operation.readingFilesDetail":"正在为临时项目准备第 {current}/{total} 个文件。","operation.importingTitle":"正在导入并识别","operation.importingDetail":"正在创建临时项目并读取受支持的资源。","operation.importedTitle":"项目导入完成","operation.preparingTitle":"正在准备生成交接包","operation.preparingDetail":"正在校验已识别的源文件并构建交接包；大型项目可能需要更长时间。","operation.validatingTitle":"正在处理已识别的项目","operation.validatingDetail":"正在提取源文件片段并写入证据，请保持此窗口打开。","operation.preparedTitle":"交接包已就绪","operation.preparedDetail":"交接包已准备好，可以进入复核。",
    ...
    const STATUS_REGISTRY = {
      ...
      handoff_ready:{family:"progress"}, awaiting_llm_responses:{family:"progress"}, pending_generation:{family:"progress"},
    ...
    function applyLanguage() {
      ...
      setOperationStepLabels(operationFlow);
    }
    ...
    function pending(reason, path) { return {kind:"pending", reason:reason || "WAITING_FOR_GENERATED_RESPONSES", path}; }
    ...
    function resultFromProjection(projection) {
      ...
      if (projection.state === "pending") return pending(projection.reason, projection.path);
    ...
    function showError(value) { $("errorMessage").textContent=value.message || String(value); $("errorRetry").hidden=!(retryAction && value.recoverable !== false); $("errorStatus").classList.add("visible"); $("errorStatus").scrollIntoView({block:"nearest",behavior:"smooth"}); }
    function clearError() { $("errorStatus").classList.remove("visible"); $("errorRetry").hidden=true; }
    function retryLastAction() { const action=retryAction; if(action) action(); }
    ...
    function setOperationStepLabels(flow) {
      const keys=flow === "handoff" ? {choose:"operation.prepare",inspect:"operation.write",ready:"operation.ready"} : {choose:"operation.choose",inspect:"operation.recognize",ready:"operation.ready"};
      document.querySelectorAll("[data-operation-step]").forEach((node)=>{const key=keys[node.dataset.operationStep];if(key){node.textContent=t(key);}});
    }
    function setProjectTransition(stage, title, detail, flow="project") {
      operationFlow=flow;
      setOperationStepLabels(flow);
    ...
    function hideProjectTransition() { const overlay=$("projectTransition"); overlay.hidden=true; overlay.classList.remove("complete"); operationFlow="project"; setOperationStepLabels(operationFlow); }
    ...
    function useInspection(data) { const routing=data.routing || {}; sourceLocaleManual=false; const sourceFiles=Array.isArray(data.source_files) ? data.source_files : (Array.isArray(routing.source_files) ? routing.source_files : (routing.supported_files || []).filter(item=>item.adapter !== "core.android-strings" || item.android_role === "source_candidate").map(item=>item.path || item)); $("sourceFiles").value=sourceFiles.join("\n"); renderRouting(routing); }
    ...
    function renderRouting(routing, preserveInputs=false) {
      ...
      const files=routing.supported_files || []; const count=Number(routing.supported_file_count || 0); const sourceFiles=Array.isArray(routing.source_files) ? routing.source_files : files.filter(item=>item.adapter !== "core.android-strings" || item.android_role === "source_candidate").map(item=>item.path || item); const sourceCount=Number(routing.source_file_count ?? sourceFiles.length); const adapters=routing.adapters || Object.entries(routing.adapter_counts || {}).map(([adapter,file_count])=>({adapter,file_count})); const suggestion=routing.source_locale_suggestion || {};
      routingReady=count > 0 && sourceCount > 0;
      if(!routingReady){const message=count > 0 && sourceCount === 0 ? t("routing.sourceFilesMissing") : (routing.reason || t("prepare.noResourcesCopy"));$("routing").innerHTML=`<div class="recognition-placeholder"><strong>${escapeHtml(t("prepare.noResources"))}</strong><p>${escapeHtml(message)}</p></div>`;$("sourceLocaleBadge").textContent=t("prepare.noSuggestion");$("sourceLocaleHint").textContent=t("prepare.noSuggestionCopy");syncWorkflowControls();return;}
      ...
      const excludedCount=Array.isArray(routing.android_locale_reference_files) ? routing.android_locale_reference_files.length : 0; const sourceNote=excludedCount ? t("routing.sourceTruthExclusion",{count:excludedCount}) : "";
      $("routing").innerHTML=`<div class="project-summary"><div class="project-signature"><span>${escapeHtml(t("routing.detectedNature"))}</span><strong>${escapeHtml(projectTypeLabel(routing.detected_project_type))}</strong>${reason}</div><div class="summary-stat"><span>${escapeHtml(t("routing.resources"))}</span><strong>${count}</strong></div><div class="summary-stat"><span>${escapeHtml(t("routing.sourceFiles"))}</span><strong>${sourceCount}</strong></div><div class="summary-stat"><span>${escapeHtml(t("routing.adapters"))}</span><strong>${adapters.length}</strong></div><div class="summary-stat"><span>${escapeHtml(t("routing.primaryRoute"))}</span><strong>${escapeHtml(adapterLabel(primary))}</strong></div></div>${sourceNote ? `<p class="safety-note">${escapeHtml(sourceNote)}</p>` : ""}${warning ? `<p class="safety-note">${escapeHtml(warning)}</p>` : ""}<details class="recognized-files"><summary>${escapeHtml(t("routing.viewFiles",{count}))}</summary><div class="table-wrap"><table><thead><tr><th>${escapeHtml(t("routing.path"))}</th><th>${escapeHtml(t("routing.adapter"))}</th></tr></thead><tbody>${files.slice(0,40).map(item=>`<tr><td>${escapeHtml(item.path || item)}</td><td>${escapeHtml(adapterLabel(item.adapter || ""))}</td></tr>`).join("")}</tbody></table>${files.length>40 ? `<p>${escapeHtml(t("routing.moreFiles",{count:files.length-40}))}</p>` : ""}<p>${escapeHtml(t("routing.preflight",{mode:preflight}))}</p></div></details>`;
      syncWorkflowControls();
    }
    async function runAgent(mode) { await runBusy(async()=>{ validateRunInputs(); retryAction=()=>runAgent(mode); setProjectTransition("choose",t("operation.preparingTitle"),t("operation.preparingDetail"),"handoff"); try { const payload=payloadBase(); if(mode==="synthetic") payload.synthetic_draft=true; if(mode==="responses"){payload.responses_dir=$("responsesDir").value.trim(); if(!payload.responses_dir) throw new WorkbenchRequestError("RESPONSES_REQUIRED",t("validation.responsesRequired"),400,true,[]);} setProjectTransition("inspect",t("operation.validatingTitle"),t("operation.validatingDetail"),"handoff"); const data=await postJson("/api/agent-run",payload); currentProject=payload.project; currentSession={...data.agent_result, run_id:data.agent_result.run_id}; await loadProjectState(currentSession.run_id); navigate("/review",currentSession.run_id); setProjectTransition("ready",t("operation.preparedTitle"),t("operation.preparedDetail"),"handoff"); await transitionPause(); setStatus(t("success.runPrepared")); retryAction=null; } finally { hideProjectTransition(); } }); }
    ...
    function renderOverview() { const projectBody=$("overviewProjectBody"); if(!currentProject){projectBody.innerHTML=`<div class="empty">${escapeHtml(t("overview.chooseProject"))}</div>`;$("nextAction").innerHTML=`<button class="primary" type="button" onclick="navigate('/generate')">${escapeHtml(t("overview.selectProject"))}</button>`;return;} const artifactResult=currentArtifactState || missing("NOT_LOADED"); const artifactStatus=artifactResult.kind === "available" ? statusText(artifactResult.data.status || "unknown") : statusText(artifactResult.kind); const latestRun=currentIndex && currentIndex.latest_session_id; const latestSession=latestRun && Array.isArray(currentIndex.sessions) ? currentIndex.sessions.find(item=>(item.run_id || item.session_id)===latestRun) : null; const pending=latestSession && ["awaiting_llm_responses","handoff_ready"].includes(String(latestSession.status || "")); projectBody.innerHTML=`<strong class="path">${escapeHtml(currentProject)}</strong><p>${latestRun ? escapeHtml(t("overview.latestRun",{runId:latestRun})) : escapeHtml(t("overview.noRuns"))}</p><p>${escapeHtml(t("overview.artifactState",{status:artifactStatus}))}</p>`; $("nextAction").innerHTML=pending ? `<button class="primary" type="button" onclick="continueLocalization('${escapeJs(latestRun)}')">${escapeHtml(t("overview.continueLocalization"))}</button>` : latestRun ? `<button class="primary" type="button" onclick="navigate('/review', currentIndex.latest_session_id)">${escapeHtml(t("overview.reviewLatest"))}</button>` : `<button class="primary" type="button" onclick="navigate('/generate')">${escapeHtml(t("overview.prepareFirst"))}</button>`; }

    function openPrepareForRun(runId) { navigate("/generate",runId); window.setTimeout(()=>{const details=document.querySelector("#prepareSection details");if(details) details.open=true;const responses=$("responsesDir");if(responses && !responses.disabled) responses.focus();},80); }
    async function continueLocalization(runId) { const project=$("project").value.trim() || currentProject; if(!project){navigate("/generate");return;} if(!routingReady) await inspectProject(); if(routingReady) openPrepareForRun(runId); }
    ...
    async function renderRunView(view) {
      ...
      renderReviewNextAction(view); renderArtifactState(artifactEvidence,view.artifacts || []); renderProjection("reviewReadiness",readinessEvidence); renderProjection("deliveryReadiness",deliveryEvidence); await renderApply(view,applyEvidence); renderRisks(view,[readinessEvidence,deliveryEvidence,applyEvidence]); renderQueue(view); renderArtifacts(view.artifacts || []); renderProviderSmoke(view);
      updateContext();
    }
    function renderReviewNextAction(view) { const pending = ["awaiting_llm_responses","handoff_ready"].includes(String(view && view.session && view.session.status || "")); $("reviewNextAction").hidden=!pending; if(pending) $("reviewNextActionCopy").textContent=t("review.awaitingActionCopy"); }
    ...
    function rawPreviewAction(result, path) { if(!path || !result || result.kind === "missing" || result.kind === "pending") return ""; if(result.data) rawArtifactPreviews[path]={raw:JSON.stringify(result.data,null,2),truncated:false}; return `<button type="button" onclick="openRawArtifactPreview('${escapeJs(path)}')">${escapeHtml(t("review.openRawArtifact"))}</button>`; }
