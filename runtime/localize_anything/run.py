...
from .provider_real_smoke import PROVIDER_REAL_SMOKE_ASSETS, PROVIDER_REAL_SMOKE_PLAN_JSON
...
def _summary(
    ...
    for key, name in (
        ...
        *tuple(PROVIDER_REAL_SMOKE_ASSETS.items()),
    ...
    summary = {
        ...
        "summary": {
            ...
            "provider_real_smoke_plan_status": (
                read_json(state_dir / PROVIDER_REAL_SMOKE_PLAN_JSON).get("status", "not_checked")
                if (state_dir / PROVIDER_REAL_SMOKE_PLAN_JSON).is_file()
                else "not_checked"
            ),
            ...
            "benchmark_comparison_status": (
                read_json(state_dir / BENCHMARK_COMPARISON_REPORT_JSON).get("status", "not_checked")
                if (state_dir / BENCHMARK_COMPARISON_REPORT_JSON).is_file()
                else "not_checked"
            ),
            "benchmark_claim_boundary_status": (
            ...
            "provider_execution_safety_status": (
                read_json(state_dir / PROVIDER_EXECUTION_SAFETY_DECISION_JSON).get("status", "not_checked")
                if (state_dir / PROVIDER_EXECUTION_SAFETY_DECISION_JSON).is_file()
                else "not_checked"
            ),
            ...
            "adapter_support_matrix_status": (
                read_json(state_dir / ADAPTER_SUPPORT_MATRIX_JSON).get("status", "not_checked")
                if (state_dir / ADAPTER_SUPPORT_MATRIX_JSON).is_file()
                else "not_checked"
            ),
